...
rb_insert_augmented_cached(struct rb_node *node,
			   struct rb_root_cached *root, bool newnodeleft,
			   const struct rb_augment_callbacks *augment)
{
	if (newnodeleft)
...
static inline void							\
RBNAME ## _copy(struct rb_node *rb_old, struct rb_node *rb_newnode)		\
{									\
...
static void								\
RBNAME ## _rotate(struct rb_node *rb_old, struct rb_node *rb_newnode)	\
{									\
	RBSTRUCT *old = rb_entry(rb_old, RBSTRUCT, RBFIELD);		\
	RBSTRUCT *newnode = rb_entry(rb_newnode, RBSTRUCT, RBFIELD);		\
	newnode->RBAUGMENTED = old->RBAUGMENTED;				\
		  ...
		  struct rb_node *parent, struct rb_root *root)
{
	if (parent) {
		if (parent->rb_left == old)
			...
			WRITE_ONCE(parent->rb_right, newnode);
	} else
