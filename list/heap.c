...
// 我觉得我需要一个特殊的函数来打印这个数组所有的内容
void printf_heap(bheap *heap)
{
    int *pi = NULL;
    for (size_t i = 0; i <= heap->size; ++i)
    {
        pi = bheap_index(heap, i);
        printf("%d, ", *pi);
    }
    printf("\n");
}
...
{
    return bheap_index(heap, bheap_root());
}

#define bheap_top(heap, type) \
    (type *)bheap_top_(heap)
...
{
    return heap->size + 1 == heap->capacity;
}
...
void bheap_fixdown(bheap *heap, size_t index)
{
    ...
    while ((child = bheap_left(index)) <= heap->size)
    {
        ...
        {
            child = (heap->predicate)(bheap_index(heap, child), bheap_index(heap, child + 1)) ? 
                child : child + 1;
        }   
...
void bheap_pop(bheap *heap)
{
    ...
    // 然后启动一个向下过滤的过程而已
    (heap->move)(bheap_index(heap, bheap_root()), bheap_index(heap, heap->size));
    --heap->size;
    // 然后启动一次下滤
    bheap_fixdown(heap, bheap_root());
...
void bheap_realloc(bheap *heap)
{
    heap->capacity = ceiling(heap->capacity + 1);

    // 现在我有一个数组，我需要重新分配一个数组
    // 我现在突然又有了一个疑惑
    // size表示的是元素的数量，其占据的内存长度为 size * size_types
    // 那capacity是什么捏？？
    // ceiling的单位也是元素的数量
